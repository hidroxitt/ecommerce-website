...
public class VNPPaymentService extends AbstractPaymentService {
    ...
    public PaymentResponse payment(CreateOrderRequest createOrderRequest) {
        ...
        total = total.multiply(AppConstant.ONE_HUNDRED).setScale(0, RoundingMode.CEILING);
        ...
        vnPayInput.setVnp_ReturnUrl(this.returnUrl + "?provider=VNPAY&uuid=" + createOrderRequest.getUuid());
        vnPayInput.setVnp_IpAddr(createOrderRequest.getIpAddress());
        Calendar calendar = Calendar.getInstance(TimeZone.getTimeZone("Asia/Ho_Chi_Minh"));
        vnPayInput.setVnp_CreateDate(DateUtil.format(calendar.getTime(), AppConstant.Date.DATE_TIME_FORMAT1));
        calendar.add(Calendar.MINUTE, this.vnPayProperties.getTtl());
        vnPayInput.setVnp_ExpireDate(DateUtil.format(calendar.getTime(), AppConstant.Date.DATE_TIME_FORMAT1));

        String queryParam = vnPayInput.toQueryParam();
        String secureHash = this.hmacSHA512(this.vnPayProperties.getSecretKey(), queryParam);
    ...
    public Order afterPayment(CreateOrderRequest createOrderRequest, Map<String, Object> data) {
        ...
        String signValue = this.hashAllFields(_data, this.vnPayProperties.getSecretKey());
        ...
        // todo: fix later
//        VnPayQueryOutput vnPayQueryOutput = this.queryTransactionVnPay(txnRef, payDate, createOrderRequest.getIpAddress());
...
//        }
    ...
    public void refund(Order order) {
        ...
        vnPayRefundInput.setVnp_Amount(order.getTotal().multiply(AppConstant.ONE_HUNDRED).setScale(0, RoundingMode.CEILING).toString());
        vnPayRefundInput.setVnp_OrderInfo("Hoan tien GD OrderId:" + order.getPaymentOnlineId());
        ...
        vnPayRefundInput.setVnp_CreateDate(DateUtil.format(cld.getTime(), AppConstant.Date.DATE_TIME_FORMAT1));
        vnPayRefundInput.setVnp_IpAddr("localhost");
        String checkSum = this.createCheckSum(vnPayRefundInput);

        String vnp_SecureHash = this.hmacSHA512(this.vnPayProperties.getSecretKey(), checkSum);
        vnPayRefundInput.setVnp_SecureHash(vnp_SecureHash);
        ResponseEntity<String> stringResponseEntity = this.defaultRestTemplate.postForEntity(this.vnPayProperties.getApi(), vnPayRefundInput, String.class);
    ...
    public VnPayQueryOutput queryTransactionVnPay(String oderId, String transDate, String ip) {
        ...
        vnPayInput.setVnp_CreateDate(DateUtil.format(new Date(), AppConstant.Date.DATE_TIME_FORMAT1));
        vnPayInput.setVnp_IpAddr(ip);
        String checkSum = this.createCheckSum(vnPayInput);
        String secureHash = this.hmacSHA512(this.vnPayProperties.getSecretKey(), checkSum);
        vnPayInput.setVnp_SecureHash(secureHash);
        ResponseEntity<String> response = this.defaultRestTemplate.postForEntity(this.vnPayProperties.getApi(), vnPayInput, String.class);
        if (!HttpStatus.OK.equals(response.getStatusCode())) {
            throw new ValidationException("Truy vấn thông tin giao dịch thất bại");
        }
        return this.objectMapper.readValue(response.getBody(), VnPayQueryOutput.class);
    }

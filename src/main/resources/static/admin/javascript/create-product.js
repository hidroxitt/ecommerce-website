...
const btnAddTechDOM = document.getElementById('btn-add-tech');
const techContainerDOM = document.getElementById('tech-container');
...
function removeSize(size) {
    ...
    document.querySelectorAll(`.size-${size}`).forEach(r => {
...
function deleteSpecifications(id) {
    const row = techContainerDOM.querySelector(`#${id}`);
    techContainerDOM.removeChild(row);
}
...
(function main() {
    ...
    btnAddTechDOM.addEventListener('click', e => {
        const allRow = techContainerDOM.querySelectorAll('.row');
        const row = document.createElement('div');
        row.classList.add('row');
        row.id = `spec-${allRow.length}`;
        row.innerHTML = `
            <div class="col-md-4">
                <input class="form-control" name="specifications[${allRow.length}].key"/>
            </div>
            <div class="col-md-4">
                <input class="form-control" name="specifications[${allRow.length}].value"/>
            </div>
            <div>
                <button class="btn btn-danger" onclick="deleteSpecifications("${row.id}")">
                    <i class="fa fa-trash"></i>
                </button>
            </div>
        `;
        techContainerDOM.appendChild(row);
    })
})()
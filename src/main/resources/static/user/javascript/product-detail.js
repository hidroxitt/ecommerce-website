...
const imageShowDOM = document.getElementById('image-show');
const productDetailImageDOM = document.querySelectorAll('.product-detail-image');

let imageShowElement = null;
...
function getSelectedSizeColor() {
    ...
    return document.querySelector(`input[type=hidden]#${selectedColor.value}-${selectedSize.dataset.id}`);
}
...
(function main(){
    ...
    productDetailImageDOM.forEach(element => {
        element.addEventListener('click', e => {
            imageShowDOM.src = element.src;
            element.parentElement.classList.add('active');
            imageShowElement?.classList.remove('active');
            imageShowElement = element.parentElement;
        })
    })
})()